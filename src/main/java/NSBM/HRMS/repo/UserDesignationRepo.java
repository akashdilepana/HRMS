...
import NSBM.HRMS.model.UserDesignation;
 ...
 */
@Repository
public interface UserDesignationRepo extends CrudRepository<UserDesignation, Integer>{
    
    @Query("SELECT `id` AS `value`,`name` AS `text` FROM `user_designation` where `name` like :search order by `name` limit 20")
